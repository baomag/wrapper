...
marker <- read.table(marker, header = TRUE)

features = subset(marker, celltype==cell)$symbol

# check clusters for certain cell type gene markers
p <- FeaturePlot(abca4, features = features, pt.size=0)
ggsave(p, filename = sprintf('%s/%s_geneFt.pdf', outdir, bname) , height=height, width=width)
