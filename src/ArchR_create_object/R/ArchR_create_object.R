...
df <- read.table(input_meta_file, header = T)
...
# create project
projorganoid1 <- ArchRProject(
	  ArrowFiles = ArrowFiles, 
		  outputDirectory = "projorganoid1",
		  copyArrows = TRUE #This is recommened so that if you modify the Arrow files you have an original copy for later usage.
			)

paste0("Memory Size = ", round(object.size(projorganoid1) / 10^6, 3), " MB")

getAvailableMatrices(projorganoid1)

saveArchRProject(ArchRProj = projorganoid1, outputDirectory = "Save-projorganoid1", load = FALSE)
